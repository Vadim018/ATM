...
namespace ATM //
{
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            InitializeForm();
            InitializeTimer();
            pictureBox2.MouseDown += new MouseEventHandler(pictureBox2_MouseDown); // захват об'єкта
            pictureBox2.MouseMove += new MouseEventHandler(pictureBox2_MouseMove); // рух об'єкта
            pictureBox2.MouseUp += new MouseEventHandler(pictureBox2_MouseUp); // відпустити об'єкт
...
/*
    -- object sender EventArgs e -- або -- object sender MouseEventArgs e -- це параметри обробника подій (event handler)

    object sender параметр представляє посилання на об'єкт, який викликав подію. У контексті події це буде об'єкт, який викликав подію
    
    EventArgs e або MouseEventArgs e параметр містить додаткову інформацію про подію, яка сталася.
    Він є об'єктом певного класу, який містить різні властивості, що вказують на конкретні деталі
    події. Наприклад, у випадку події миші (MouseEventArgs), цей об'єкт може містити інформацію про
    положення курсора, кнопку миші, яка була натиснута тощо.
 
*****

    ref Message msg - параметр методу, який приймає об'єкт Message по посиланню (за допомогою ключового слова ref).
    Message - це структура, яка представляє повідомлення від вікна Windows, таке як клавішні події, мишині події тощо.

    Keys keyData: Це параметр методу, який приймає значення перерахування Keys. 
    Keys - це перерахування, яке представляє клавішу на клавіатурі. В цьому 
    контексті keyData містить інформацію про ту клавішу, яка була натиснута користувачем.

*****

    Структура DateTime використовується для представлення дати і часу в програмах на платформі .NET. 
    Вона містить методи для роботи з датою та часом, такі як отримання поточної дати та часу (DateTime.Now), 
    конвертація дати у різні формати, обчислення різниці між датами, тощо.
